...
export function buildEmbedScriptTag(options: {
  ...
  const integrity = options.integrity ?? AGENT_JS_INTEGRITY;
  return `<script src="${options.src}" data-key="${options.key}" integrity="${integrity}" crossorigin="anonymous" async></script>`;
}
