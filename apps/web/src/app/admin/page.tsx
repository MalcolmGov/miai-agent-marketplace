...
type AdminPayload = {
  ...
  economics: {
    ...
    tokenRevenueDeltaPct: number | null;
    prepaidCollectedUsd?: number;
    rentalMrr: number;
...
export default function AdminPage() {
  ...
  const tokenSub =
    (k.prepaidCollectedUsd ?? 0) > 0
      ? `${formatUsd(k.prepaidCollectedUsd!)} prepaid collected`
      : e.tokenRevenueDeltaPct != null
        ? `${e.tokenRevenueDeltaPct >= 0 ? "↑" : "↓"} ${Math.abs(e.tokenRevenueDeltaPct)}% vs last month — metered usage`
        : "metered usage from live turns this month";
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm text-[var(--muted)]">
            Every agent customers have rented, live status, and monthly rental revenue — wired to the
            live marketplace store.
          </p>
        </div>
        <div className="flex items-center gap-2">
          ...
          {data.source === "empty" && (
            <span className="chip px-3 py-1 text-[11px] font-semibold uppercase tracking-wider">
              Awaiting rentals
            </span>
          )}
          <span className="chip chip-live px-3 py-1 text-[11px] font-semibold uppercase tracking-wider">
        ...
        {[
          {
            ...
            sub: `across ${k.customers} customer${k.customers === 1 ? "" : "s"}`,
          },
          {
            label: "Monthly rental revenue",
            value: formatUsd(k.monthlyRentalRevenueDisplay),
            sub: "list-price MRR • live & rented",
          },
          {
            label: "Tokens consumed",
            value: tokensLabel,
            sub: "this month • from audit debits",
          },
          {
            label: "Custom requests",
            value: String(k.customRequests),
            sub:
              k.customRequestsNew > 0
                ? `${k.customRequestsNew} new to triage`
                : "no open requests",
          },
        ].map((card) => (
        ...
        <div className="space-y-5 p-5 sm:p-6">
          <div>
            ...
            <p className="mt-1 max-w-3xl text-sm text-[var(--muted)]">
              Token usage and agent rentals across the live partner base — rental MRR from catalogue
              list prices, token revenue from metered debits
              {(k.prepaidCollectedUsd ?? 0) > 0 ? " and prepaid top-ups" : ""}.
            </p>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-5">
            ...
            >
              ...
              <div className="mt-1 text-xs text-[var(--muted)]">{tokenSub}</div>
            </div>
            ...
            <div className="panel p-4">
              ...
              <div className="mt-1 text-xs text-[var(--muted)]">
                {e.activeBusinesses === 0 ? "no live rentals yet" : "with a live or rented agent"}
              </div>
            </div>
            <div className="panel p-4">
              <div className="text-[11px] font-semibold uppercase tracking-wider text-[var(--muted)]">
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
            ...
            <div className="panel p-4 sm:col-span-2 xl:col-span-4">
              <div className="mb-2 flex flex-wrap items-baseline justify-between gap-2">
                ...
                <span className="text-xs text-[var(--muted)]">last 6 months — from metered usage</span>
              </div>
              <div className="h-[220px]">
                <LineTrendChart
                  points={e.tokenTrend}
                  valueLabel={formatUsd(e.tokenTrend.at(-1)?.value ?? 0)}
                />
              </div>
            </div>
          </div>
